...
void	move_player(t_mlx *mlx, double move_x, double move_y) // move the player
{
	...
	if (mlx->dt->map2d[map_grid_y][map_grid_x] != '1'
		&& mlx->dt->map2d[(int)(new_y * inv_TILE_SIZE)][(int)(new_x
			* inv_TILE_SIZE)] != '1')
	{
...
mlx_texture_t	*get_south_wall_color(t_mlx *mlx)
{
	...
	; // Color for the south wall
}

mlx_texture_t	*get_north_wall_color(t_mlx *mlx)
{
	return (mlx->texture.so);
	; // Color for the north wall
}
...
double	get_x_o(mlx_texture_t *texture, t_mlx *mlx, double wall_distance)
{
	...
	if (mlx->ray->flag == 1)
		x_o = fmod((mlx->ply->plyr_x + wall_distance * cos(mlx->ray->ray_ngl))
			* (texture->width / TILE_SIZE), texture->width);
	else
		x_o = fmod((mlx->ply->plyr_y + wall_distance * sin(mlx->ray->ray_ngl))
			* (texture->width / TILE_SIZE), texture->width);
	return (x_o);
}
...
void	draw_wall(t_mlx *mlx, int t_pix, int b_pix, double wall_h)
{
	...
	x_o = get_x_o(texture, mlx, mlx->ray->distance);
...
void	render_wall(t_mlx *mlx, int ray)
{
	...
	cos_val = cos(nor_angle(mlx->ray->ray_ngl - mlx->ply->angle));
	tan_val = tan(mlx->ply->fov_rd / 2);
	mlx->ray->distance *= cos_val;                                    
		// fix the fisheye
	wall_h = (TILE_SIZE / mlx->ray->distance) * ((S_W / 2) / tan_val);
		// get the wall height
...
float	get_h_inter(t_mlx *mlx, float angl)
{
	...
	delta_x = h_x - mlx->ply->plyr_x;
	delta_y = h_y - mlx->ply->plyr_y;
...
void	move_to_next_intersection(float *v_x, float *v_y, float *x_step,
		float *y_step, t_mlx *mlx, int pixel)
{
...
void	start_the_game(t_data *dt, t_game *game)
{
	t_mlx	mlx;
...
// gcc raycast.c ./libs/MLX42/build/libmlx42.a ./libs/libft/libft.a -Iinclude
	-ldl -lglfw -pthread -lm -fsanitize=address -o navid